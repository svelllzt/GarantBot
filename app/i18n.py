...
RU = {
    ...
    "withdraw_ok": "✅ Заявка на вывод #{id}: <b>{amount} {currency}</b> на <code>{details}</code>. Админ подтвердит — перевод уйдёт с кошелька гаранта.",
    ...
    "deal_cancel_wait": "Ждём согласие второй стороны.",
    ...
    "admin_wd_ok": "✅ Вывод #{id} отправлен: <b>{amount} {currency}</b> на <code>{address}</code>.\nTX: <code>{hash}</code>",
    "admin_wd_no": "Вывод отклонён, сумма возвращена на баланс.",
    "admin_wd_sending": "Отправляю с кошелька гаранта…",
    "admin_wd_fail": "⚠️ Не удалось отправить вывод #{id}. Заявка снова в очереди. Проверьте баланс кошелька и сеть. Если перевод уже ушёл в эксплорере — не подтверждайте повторно.",
    "admin_wd_no_wallet": "Кошелёк V4 не подключён. Задайте mnemonic в [ton].",
    "admin_wd_bad_addr": "В заявке некорректный TON-адрес. Отклоните её, чтобы вернуть баланс.",
    "deposit_rejected": "❌ Заявка на пополнение #{id} ({amount} {currency}) отклонена.",
    "withdraw_sent": "✅ Вывод #{id}: <b>{amount} {currency}</b> отправлен на <code>{address}</code>.\nTX: <code>{hash}</code>",
    "withdraw_rejected": "❌ Заявка на вывод #{id} отклонена. <b>{amount} {currency}</b> вернулись на баланс.",
    "withdraw_busy": "⚠️ У вас уже есть заявка на вывод. Дождитесь решения администратора.",
    ...
    "deal_ask_group": "🛡️ <b>Категория сделки</b>",
    ...
    "channel_listing_nft_buy": (
        ...
        "💰 Готов заплатить: <b>{amount}</b>\n\n"
        "{desc}\n\n"
        "Продавец сначала кладёт NFT на банк. Оплата TON или USDT через бота-гаранта. Нажмите «Открыть в боте»."
    ),
    ...
    "deal_photo": "Фото",
    "deal_chat": "Чат",
    "deal_chat_ask": "Напишите сообщение второй стороне или пришлите фото. Можно несколько подряд. Когда закончите — «Готово».",
    "deal_chat_ok": "Сообщение отправлено.",
    "deal_chat_peer": "💬 Сообщение по сделке #{id} от {who}:\n{text}",
    "deal_thread_seller": "Продавец @{who}",
    "deal_thread_buyer": "Покупатель @{who}",
    "deal_thread_admin": "Админ",
    "deal_thread_admin_seller": "Админ → продавец",
    "deal_thread_admin_buyer": "Админ → покупатель",
    "admin_write_seller": "Написать продавцу",
    "admin_write_buyer": "Написать покупателю",
    "admin_ask_write_seller": "Сообщение продавцу по сделке #{id}. Покупатель его не увидит. Текст или фото.",
    "admin_ask_write_buyer": "Сообщение покупателю по сделке #{id}. Продавец его не увидит. Текст или фото.",
    "admin_wrote": "Сообщение отправлено.",
    ...
    "admin_cfg_required_channel": "Канал обязательной подписки (@name или -100id)",
...
EN = {
    ...
    "withdraw_ok": "✅ Withdrawal #{id}: <b>{amount} {currency}</b> to <code>{details}</code>. An admin will approve it, then the escrow wallet sends the coins.",
    ...
    "deal_cancel_wait": "Waiting for the other party to agree.",
    ...
    "admin_wd_ok": "✅ Withdrawal #{id} sent: <b>{amount} {currency}</b> to <code>{address}</code>.\nTX: <code>{hash}</code>",
    "admin_wd_no": "Withdrawal rejected, funds returned.",
    "admin_wd_sending": "Sending from the escrow wallet…",
    "admin_wd_fail": "⚠️ Could not send withdrawal #{id}. The ticket is back in the queue. Check wallet balance and the network. If the transfer already appears in the explorer, do not approve it again.",
    "admin_wd_no_wallet": "Wallet V4 is not connected. Set the mnemonic in [ton].",
    "admin_wd_bad_addr": "This ticket has an invalid TON address. Reject it to return the balance.",
    "deposit_rejected": "❌ Deposit request #{id} ({amount} {currency}) was rejected.",
    "withdraw_sent": "✅ Withdrawal #{id}: <b>{amount} {currency}</b> sent to <code>{address}</code>.\nTX: <code>{hash}</code>",
    "withdraw_rejected": "❌ Withdrawal #{id} was rejected. <b>{amount} {currency}</b> returned to your balance.",
    "withdraw_busy": "⚠️ You already have a withdrawal request. Wait for an admin decision.",
    ...
    "deal_ask_group": "🛡️ <b>Deal category</b>",
    ...
    "channel_listing_nft_buy": (
        ...
        "💰 Will pay: <b>{amount}</b>\n\n"
        "{desc}\n\n"
        "The seller must put the NFT on the bank first. Pay TON or USDT through the bot. Tap Open in bot."
    ),
    ...
    "deal_photo": "Photo",
    "deal_chat": "Chat",
    "deal_chat_ask": "Write a message to the other party or send a photo. You can send several. Tap Done when finished.",
    "deal_chat_ok": "Message sent.",
    "deal_chat_peer": "💬 Message on deal #{id} from {who}:\n{text}",
    "deal_thread_seller": "Seller @{who}",
    "deal_thread_buyer": "Buyer @{who}",
    "deal_thread_admin": "Admin",
    "deal_thread_admin_seller": "Admin → seller",
    "deal_thread_admin_buyer": "Admin → buyer",
    "admin_write_seller": "Message seller",
    "admin_write_buyer": "Message buyer",
    "admin_ask_write_seller": "Message to the seller on deal #{id}. The buyer will not see it. Text or photo.",
    "admin_ask_write_buyer": "Message to the buyer on deal #{id}. The seller will not see it. Text or photo.",
    "admin_wrote": "Message sent.",
    ...
    "admin_cfg_required_channel": "Required subscribe channel (@name or -100id)",
