...
from app.keyboards import NavCB, WalletCB, cancel_kb, deposit_kb, lang_kb, requisites_kb, withdraw_method_kb
...
async def ask_card(call: CallbackQuery, state: FSMContext, lang: str, theme: Theme):
    await state.set_state(Requisites.card)
    await call.message.answer(t(lang, "req_ask_card"), reply_markup=cancel_kb(lang, theme))
    await call.answer()


@router.callback_query(NavCB.filter(F.a == "req_phone"))
async def ask_phone(call: CallbackQuery, state: FSMContext, lang: str, theme: Theme):
    await state.set_state(Requisites.phone)
    await call.message.answer(t(lang, "req_ask_phone"), reply_markup=cancel_kb(lang, theme))
    await call.answer()
...
async def save_card(message: Message, state: FSMContext, db: Storage, lang: str, theme: Theme):
...
async def save_phone(message: Message, state: FSMContext, lang: str, theme: Theme):
    if is_cancel(message.text or ""):
        return
    ...
    await message.answer(t(lang, "req_ask_bank"), reply_markup=cancel_kb(lang, theme))


@router.message(Requisites.bank)
async def save_bank(message: Message, state: FSMContext, db: Storage, lang: str, theme: Theme):
    if is_cancel(message.text or ""):
        return
...
async def ask_deposit(call: CallbackQuery, state: FSMContext, lang: str, settings: Settings, theme: Theme):
...
async def ask_withdraw(call: CallbackQuery, state: FSMContext, lang: str, settings: Settings, theme: Theme):
...
async def withdraw_amount(message: Message, state: FSMContext, db: Storage, lang: str, settings: Settings, theme: Theme):
    ...
    await message.answer(t(lang, "withdraw_method"), reply_markup=withdraw_method_kb(lang, theme))
