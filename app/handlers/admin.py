...
from app.keyboards import AdminCB, admin_kb, dispute_admin_kb, ticket_kb
...
from app.util import is_cancel, money, parse_amount
...
async def admin_entry(message: Message, lang: str, settings: Settings):
    if not _admin(settings, message.from_user.id):
        await message.answer(t(lang, "admin_only"))
        return
    await message.answer(t(lang, "admin_menu"), reply_markup=admin_kb(lang))


@router.callback_query(AdminCB.filter(F.a == "stats"))
async def stats(call: CallbackQuery, db: Storage, lang: str, settings: Settings):
...
async def ask_ban(call: CallbackQuery, state: FSMContext, lang: str, settings: Settings):
    if not _admin(settings, call.from_user.id):
        return
...
async def ask_unban(call: CallbackQuery, state: FSMContext, lang: str, settings: Settings):
    if not _admin(settings, call.from_user.id):
        return
...
async def disputes(call: CallbackQuery, db: Storage, lang: str, settings: Settings):
    if not _admin(settings, call.from_user.id):
        return
    ...
    for deal in rows:
        ...
        await call.message.answer(
            ...
            reply_markup=dispute_admin_kb(lang, deal["id"]),
        )
    await call.answer()
...
async def deposits(call: CallbackQuery, db: Storage, lang: str, settings: Settings):
    if not _admin(settings, call.from_user.id):
        return
    ...
    for row in rows:
        await call.message.answer(
            ...
            reply_markup=ticket_kb("dep", row["id"], lang),
        )
    await call.answer()


@router.callback_query(AdminCB.filter(F.a == "wds"))
async def withdraws(call: CallbackQuery, db: Storage, lang: str, settings: Settings):
    if not _admin(settings, call.from_user.id):
        return
    ...
    for row in rows:
        await call.message.answer(
            ...
            reply_markup=ticket_kb("wd", row["id"], lang),
        )
    await call.answer()
