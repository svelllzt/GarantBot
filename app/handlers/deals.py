...
DEAL_BTN = {t("ru", "btn_deal"), t("en", "btn_deal")}
HISTORY_BTN = {t("ru", "btn_history"), t("en", "btn_history")}


def _active_label(text: str) -> bool:
    ru = t("ru", "active_deal_btn", id=0).split("#")[0]
    en = t("en", "active_deal_btn", id=0).split("#")[0]
    return text.startswith(ru) or text.startswith(en)


async def _show_deal(bot, db: Storage, deal, user_id: int, lang: str, settings: Settings, message=None, edit=False):
    text = await render_deal(db, deal, lang, settings.currency)
    markup = deal_kb(lang, deal, user_id)
...
@router.message(F.text.in_(DEAL_BTN))
async def new_deal(message: Message, db: Storage, lang: str, settings: Settings):
    if not message.from_user.username:
        await message.answer(t(lang, "need_username"))
        return
    active = await db.active_deal(message.from_user.id)
    if active:
        await _show_deal(message.bot, db, active, message.from_user.id, lang, settings, message=message)
        return
    await message.answer(t(lang, "deal_role"), reply_markup=role_kb(lang))


@router.message(F.text.func(lambda x: isinstance(x, str) and _active_label(x)))
async def open_active(message: Message, db: Storage, lang: str, settings: Settings):
    active = await db.active_deal(message.from_user.id)
    if not active:
        await message.answer(t(lang, "menu"), reply_markup=main_menu(lang))
        return
    await _show_deal(message.bot, db, active, message.from_user.id, lang, settings, message=message)


@router.callback_query(DealCB.filter(F.a == "role"))
async def pick_role(call: CallbackQuery, callback_data: DealCB, state: FSMContext, lang: str):
    ...
    await call.message.answer(t(lang, "deal_ask_user"))
    await call.answer()


@router.message(DealFlow.username)
async def find_peer(message: Message, state: FSMContext, db: Storage, lang: str):
    ...
    await message.answer(
        ...
        reply_markup=preview_kb(lang),
    )


@router.callback_query(DealCB.filter(F.a == "abort"))
async def abort_preview(call: CallbackQuery, state: FSMContext, db: Storage, lang: str):
    await state.clear()
    active = await db.active_deal(call.from_user.id)
    await call.message.edit_text(t(lang, "cancelled"))
    await call.message.answer(t(lang, "menu"), reply_markup=main_menu(lang, active["id"] if active else None))
    await call.answer()
...
async def send_offer(call: CallbackQuery, state: FSMContext, db: Storage, lang: str, settings: Settings):
    data = await state.get_data()
    peer_id = data.get("peer_id")
    as_buyer = data.get("as_buyer", True)
    if not peer_id:
    ...
    await call.message.edit_text(t(lang, "deal_sent"))
    await call.message.answer(t(lang, "menu"), reply_markup=main_menu(lang, deal_id))
    await call.bot.send_message(
        ...
        reply_markup=offer_kb(peer_lang, deal_id),
    )
    await call.answer()


@router.callback_query(DealCB.filter(F.a == "acc"))
async def accept_offer(call: CallbackQuery, callback_data: DealCB, db: Storage, lang: str, settings: Settings):
    try:
    ...
    for uid in (deal["seller_id"], deal["buyer_id"]):
        ...
        await call.bot.send_message(
            uid,
            text,
            reply_markup=deal_kb(user_lang, deal, uid),
        )
        await call.bot.send_message(uid, t(user_lang, "menu"), reply_markup=main_menu(user_lang, deal["id"]))
    await call.answer()


@router.callback_query(DealCB.filter(F.a == "dec"))
async def decline_offer(call: CallbackQuery, callback_data: DealCB, db: Storage, lang: str):
    deal = await db.get_deal(callback_data.i)
    try:
    ...
    await call.bot.send_message(other, t(other_lang, "deal_declined_peer"), reply_markup=main_menu(other_lang))
    await call.message.answer(t(lang, "menu"), reply_markup=main_menu(lang))
    await call.answer()


@router.callback_query(DealCB.filter(F.a == "open"))
async def reopen(call: CallbackQuery, callback_data: DealCB, db: Storage, lang: str, settings: Settings):
    deal = await db.get_deal(callback_data.i)
    if deal is None:
        await call.answer(t(lang, "error"), show_alert=True)
        return
    await _show_deal(call.bot, db, deal, call.from_user.id, lang, settings, message=call.message, edit=True)
    await call.answer()


@router.callback_query(DealCB.filter(F.a == "price"))
async def ask_price(call: CallbackQuery, callback_data: DealCB, state: FSMContext, db: Storage, lang: str, settings: Settings):
    deal = await db.get_deal(callback_data.i)
    ...
    await call.message.answer(t(lang, "deal_ask_price", currency=settings.currency))
    await call.answer()


@router.message(DealFlow.price)
async def save_price(message: Message, state: FSMContext, db: Storage, lang: str, settings: Settings):
    if is_cancel(message.text or ""):
        return
    ...
    for uid in (deal["seller_id"], deal["buyer_id"]):
        user = await db.get_user(uid)
        user_lang = user["lang"] or "ru"
        await _show_deal(message.bot, db, deal, uid, user_lang, settings)


@router.callback_query(DealCB.filter(F.a == "desc"))
async def ask_desc(call: CallbackQuery, callback_data: DealCB, state: FSMContext, db: Storage, lang: str):
    deal = await db.get_deal(callback_data.i)
    if deal is None or deal["seller_id"] != call.from_user.id or deal["status"] != DEAL_OPEN:
        await call.answer(t(lang, "error"), show_alert=True)
        return
    ...
    await state.update_data(deal_id=deal["id"])
    await call.message.answer(t(lang, "deal_ask_desc"))
    await call.answer()


@router.message(DealFlow.description)
async def save_desc(message: Message, state: FSMContext, db: Storage, lang: str, settings: Settings):
    if is_cancel(message.text or ""):
        return
    ...
    deal = await db.get_deal(data["deal_id"])
    await _show_deal(message.bot, db, deal, message.from_user.id, lang, settings, message=message)


@router.callback_query(DealCB.filter(F.a == "nft"))
async def pick_nft(call: CallbackQuery, callback_data: DealCB, db: Storage, lang: str):
    ...
    await call.message.edit_text(
        ...
        reply_markup=nft_pick_kb(lang, callback_data.i, items),
    )
    await call.answer()


@router.callback_query(DealCB.filter(F.a == "nftset"))
async def set_nft(call: CallbackQuery, callback_data: DealCB, db: Storage, lang: str, settings: Settings):
    try:
...
async def pay_deal(call: CallbackQuery, callback_data: DealCB, db: Storage, lang: str, settings: Settings, bank):
    try:
    ...
    await _show_deal(call.bot, db, await db.get_deal(deal["id"]), call.from_user.id, lang, settings, message=call.message)
    ...
    await _show_deal(call.bot, db, await db.get_deal(deal["id"]), deal["seller_id"], seller_lang, settings)
    await call.answer()


@router.callback_query(DealCB.filter(F.a == "ok"))
async def confirm_ask(call: CallbackQuery, callback_data: DealCB, lang: str):
    await call.message.answer(
        t(lang, "deal_confirm_ask"),
        reply_markup=confirm_kb(lang, "okgo", callback_data.i),
    )
    await call.answer()
...
async def confirm_go(
    ...
    await call.message.answer(t(lang, "deal_review_ask"), reply_markup=skip_review_kb(lang, deal["id"]))
    await call.bot.send_message(
        deal["seller_id"],
        t(
            seller_lang,
        ...
        reply_markup=main_menu(seller_lang),
    )
    await call.answer()


@router.message(DealFlow.review)
async def save_review(message: Message, state: FSMContext, db: Storage, lang: str):
    if is_cancel(message.text or ""):
        return
    data = await state.get_data()
...
async def skip_review(call: CallbackQuery, callback_data: DealCB, state: FSMContext, db: Storage, lang: str):
...
async def cancel_ask(call: CallbackQuery, callback_data: DealCB, db: Storage, lang: str):
    deal = await db.get_deal(callback_data.i)
    if deal is None:
        await call.answer(t(lang, "error"), show_alert=True)
        return
    ...
    if deal["status"] == DEAL_PENDING:
        ...
        await call.bot.send_message(other, t(other_user["lang"] or "ru", "deal_declined_peer"), reply_markup=main_menu(other_user["lang"] or "ru"))
        await call.message.answer(t(lang, "menu"), reply_markup=main_menu(lang))
        await call.answer()
        return
    await call.message.answer(t(lang, "deal_cancel_ask"), reply_markup=confirm_kb(lang, "cansend", deal["id"]))
    await call.answer()


@router.callback_query(DealCB.filter(F.a == "cansend"))
async def cancel_send(call: CallbackQuery, callback_data: DealCB, db: Storage, lang: str):
    if callback_data.x != 1:
        await call.answer()
        return
    deal = await db.get_deal(callback_data.i)
    ...
    await call.bot.send_message(
        ...
        reply_markup=peer_cancel_kb(other_lang, deal["id"]),
    )
    await call.answer()


@router.callback_query(DealCB.filter(F.a == "canok"))
async def cancel_ok(call: CallbackQuery, callback_data: DealCB, db: Storage, lang: str):
    deal = await db.get_deal(callback_data.i)
    try:
    ...
    for uid in (deal["seller_id"], deal["buyer_id"]):
        user = await db.get_user(uid)
        await call.bot.send_message(uid, t(user["lang"] or "ru", "deal_cancel_ok"), reply_markup=main_menu(user["lang"] or "ru"))
    await call.answer()
...
async def dispute(call: CallbackQuery, callback_data: DealCB, db: Storage, lang: str, settings: Settings):
    try:
    ...
    from app.keyboards import dispute_admin_kb

    for admin_id in settings.admins:
        try:
            await call.bot.send_message(admin_id, admin_text, reply_markup=dispute_admin_kb("ru", deal["id"]))
        except Exception:
            pass
...
@router.message(F.text.in_(HISTORY_BTN))
async def history(message: Message, lang: str):
    await message.answer(t(lang, "history_role"), reply_markup=history_kb(lang))


@router.callback_query(NavCB.filter(F.a.in_({"hist_s", "hist_b"})))
async def history_list(call: CallbackQuery, callback_data: NavCB, db: Storage, lang: str, settings: Settings):
